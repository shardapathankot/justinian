...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<tbody>
		<tr>
			<td>
				<p>
					<?php printf(__('You have selected the %s option for the Gazette of Law &amp; Journalism.', 'pmpro'), $pmpro_level->name); ?>
				</p>
				<p>
					<?php
						echo explode(' ', pmpro_getLevelCost($pmpro_level, true, true))[0] . ' for 12 months' . " - full access to all of GLJ's content plus regular updates with our electronic newsletter.";
					?>
				</p>
