...
function glj_state_filter() {
global $wpdb;
$states = $wpdb->get_col("SELECT 'All' UNION SELECT meta_value FROM $wpdb->postmeta WHERE meta_key = 'wpcf-state'" );
	...
	<?php
       ...
       foreach ($states as $state) {
       	 $selected = ($state == get_query_var( 'state', 'All' )) ? ' selected="selected"' : '';
...
function UpdateQueryString(key, value, url) {
    ...
    if (re.test(url)) {
		...
		jQuery('#state').change(function() {
		    window.location = UpdateQueryString('state',jQuery('#state').find(":selected").text());
		});
	</script>
<?php
}
		...
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( __( 'Older Comments', 'twentyfifteen' ) ) ) :
					printf( '<div class="1nav-previous">%s</div>', $prev_link );
				endif;
